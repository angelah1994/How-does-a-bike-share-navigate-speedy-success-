...
setwd("C:/Users/Admin/Desktop/case study 1")
library(tidyverse)
library(conflicted)
library(lubridate)
...
Annual members are more than casual members.
```{r}
# Generate a frequency table for the member_casual column
member_casual_count <- table(all_trips$member_casual)
...
all_trips_v2 %>%
mutate(weekday = wday(started_at, label = TRUE)) %>% #creates weekday field usingwday()
group_by(member_casual, weekday) %>% #groups by usertype and weekday
summarise(number_of_rides = n() #calculates the number of rides and average duration
,average_duration = mean(ride_length)) %>% # calculates the average duration
arrange(member_casual, weekday) # sorts
# Let's visualize the number of rides by rider type
all_trips_v2 %>%mutate(weekday = wday(started_at, label = TRUE)) %>%group_by(member_casual, weekday) %>%summarise(number_of_rides = n(),average_duration = mean(ride_length)) %>%arrange(member_casual, weekday) %>%ggplot(aes(x = weekday, y = number_of_rides, fill = member_casual)) +
